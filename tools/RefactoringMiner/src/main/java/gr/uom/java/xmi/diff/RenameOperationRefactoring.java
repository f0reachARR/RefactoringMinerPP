...
public class RenameOperationRefactoring implements Refactoring {
	...
	public RefactoringScope getRefactoringScope() {
		LocationInfo opLoc = renamedOperation.getLocationInfo();
		LocationInfo newLoc = new LocationInfo(  //rename op changes only one line
				opLoc.getFilePath(),
				opLoc.getStartOffset(),
				opLoc.getStartOffset() + 1,
				1,
				opLoc.getStartLine(),
				opLoc.getStartColumn(),
				opLoc.getStartLine(),
				opLoc.getStartColumn() + 1,
				opLoc.getCodeElementType()
		);

		return new RefactoringScope()
				.addAffectedLocation(newLoc)
				.addAffectedLocations(callReferences.stream().map(t -> t.getInvokedOperationAfter().getLocationInfo()).collect(Collectors.toList()));
	}
}
