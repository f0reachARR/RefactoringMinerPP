...
public class SplitAttributeRefactoring implements Refactoring, ReferenceBasedRefactoring {
	...
	public RefactoringScope getRefactoringScope() {
		RefactoringScope scope = new RefactoringScope()
				.addAffectedLocation(oldAttribute.getLocationInfo())
				.addAffectedLocations(splitAttributes.stream().map(t -> t.getLocationInfo()).collect(Collectors.toList()));

		splitAttributes.forEach(t ->
				scope.addAffectedLocations(
						t.getVariableDeclaration().getScope().getStatementsInScopeUsingVariable().stream().map(s -> s.getLocationInfo()).collect(Collectors.toList())
				)
		);
