...
public class RenameClassRefactoring implements Refactoring {
	...
	public RefactoringScope getRefactoringScope() {
		LocationInfo classLoc = renamedClass.getLocationInfo();
		LocationInfo newLoc = new LocationInfo(  //rename class changes only one line
				classLoc.getFilePath(),
				classLoc.getStartOffset(),
				classLoc.getStartOffset() + 1,
				1,
				classLoc.getStartLine(),
				classLoc.getStartColumn(),
				classLoc.getStartLine(),
				classLoc.getStartColumn() + 1,
				classLoc.getCodeElementType()
		);

		return new RefactoringScope().addAffectedLocation(newLoc);
	}
}
