...
public class ChangeTypeDeclarationKindRefactoring implements Refactoring {
	...
	public RefactoringScope getRefactoringScope() {
		LocationInfo loc1 = classAfter.getLocationInfo();

		LocationInfo loc = new LocationInfo(
				loc1.getFilePath(),
				loc1.getStartOffset(),
				loc1.getStartOffset() + changedTypeDeclarationKind.length(),
				changedTypeDeclarationKind.length(),
				loc1.getStartLine(),
				loc1.getStartColumn(),
				loc1.getStartLine(),
				loc1.getStartColumn() + changedTypeDeclarationKind.length(),
				loc1.getCodeElementType()
		);

		return new RefactoringScope().addAffectedLocation(loc);
	}
}
