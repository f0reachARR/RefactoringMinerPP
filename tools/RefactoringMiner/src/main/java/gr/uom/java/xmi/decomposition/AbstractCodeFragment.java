...
@JsonSubTypes({
		@JsonSubTypes.Type(value = AbstractExpression.class, name = "AbstractExpression"),
...
public abstract class AbstractCodeFragment implements LocationInfoProvider {
	...
	public static final Map<Integer, AbstractCodeFragment> allCodeFragments = new LinkedHashMap<>();

	public static AbstractCodeFragment get(int id)
	{
		if (id == -1)
			return null;

		AbstractCodeFragment x = allCodeFragments.get(id);

		if (x == null)
			throw new java.lang.RuntimeException("AbstractCodeFragment not found");
