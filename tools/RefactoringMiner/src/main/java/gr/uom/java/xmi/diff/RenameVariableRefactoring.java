...
public class RenameVariableRefactoring implements Refactoring, ReferenceBasedRefactoring {
	...
	public RefactoringScope getRefactoringScope() {
		LocationInfo varLoc = renamedVariable.getLocationInfo();
		LocationInfo newLoc = new LocationInfo(  //rename var changes only one line
				varLoc.getFilePath(),
				varLoc.getStartOffset(),
				varLoc.getStartOffset() + 1,
				1,
				varLoc.getStartLine(),
				varLoc.getStartColumn(),
				varLoc.getStartLine(),
				varLoc.getStartColumn() + 1,
				varLoc.getCodeElementType()
		);

		return new RefactoringScope()
				.addAffectedLocation(newLoc)
				.addAffectedLocations(variableReferences.stream().map(t -> t.getFragment2().getLocationInfo()).collect(Collectors.toList()));
	}
}
