...
public class UMLAttributeDiff {
	...
	private Set<Refactoring> getModifierRefactorings() {
		...
		List<UMLModifier> addedModifiers = UMLModifierListDiff.getAddedModifiers(
				removedAttribute.getVariableDeclaration().getModifiers(),
				addedAttribute.getVariableDeclaration().getModifiers());

		for (UMLModifier m : addedModifiers) {
			refactorings.add(new AddAttributeModifierRefactoring(m.getKeyword(), removedAttribute, addedAttribute));
		}

		List<UMLModifier> removedModifiers = UMLModifierListDiff.getRemovedModifiers(
				removedAttribute.getVariableDeclaration().getModifiers(),
				addedAttribute.getVariableDeclaration().getModifiers());

		for (UMLModifier m : removedModifiers) {
			refactorings.add(new RemoveAttributeModifierRefactoring(m.getKeyword(), removedAttribute, addedAttribute));
		}
