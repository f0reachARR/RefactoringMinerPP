...
public class CollapseHierarchyRefactoring implements Refactoring {
	...
	public RefactoringScope getRefactoringScope() {
		...
		for (UMLAttribute attr : collapsedClass.getAttributes()) {
			for (UMLAttribute attrNew : superclassAfterCollapse.getAttributes()) {
				if (attr.getName().equals(attrNew.getName())) {
					scope.addAffectedLocation(attrNew.getLocationInfo());
					break;
				}
			}
		}

		for (UMLOperation op : collapsedClass.getOperations()) {
			for (UMLOperation opNew : superclassAfterCollapse.getOperations()) {
				if (op.compareTo(opNew) == 0) {
					scope.addAffectedLocation(opNew.getLocationInfo());
					break;
				}
			}
		}
